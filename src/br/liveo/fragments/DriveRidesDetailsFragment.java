...
public class DriveRidesDetailsFragment extends Fragment{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.summeryupcoming, container, false);
		
		rootView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,LayoutParams.MATCH_PARENT ));		
		
		progressView = (RelativeLayout)rootView.findViewById(R.id.progressdialogview);
		listCurrentRides = (ListView)rootView.findViewById(R.id.upcominglist);
	...
	private void loadData() {
		...
		try {
			...
			//requestParams.put("typeData","Future");
			...
			for(int i=0;i<jsonArray.length();i++){
				...
				RideModel ridesModel = new RideModel();
				 ...
				 try{
				   ...
				   try{
					   ridesModel.setTripId(ridesJsonObject.getString("tripID"));
				   }catch(JSONException e){
					   e.printStackTrace();
				   }
